...
b) B.	Test environment docking is not supported at present, please register to create a wallet according to ** Access Guidelines **, and get the API related information of production environment.
...
5）5) Whether to enable the automatic collection function? After the function is enabled, the system will automatically transfer the user's recharge funds to the merchant collection account.
...
2）2) The wallet public key：rsa_wallet_pub。
